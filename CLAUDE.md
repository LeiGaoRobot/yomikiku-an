...
| `yomikikuan-analysis` | `analysis` | 30-day TTL + LRU-500 | Analyzer cache. **Namespace via `providerId`**: `gemini` (per-sentence analyze), `jlpt` (question payloads), `article-summary`, `translate-zh` (bilingual per-line translation), `yt-transcript` / `yt-transcript-timed` / `yt-jlpt` (YouTube import — videoId-keyed). All safe to drop. |
