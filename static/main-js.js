...
const headerSpeedValue = $('headerSpeedValue');
  ...
  const PWA_CACHE_PREFIX = 'yomikikuan-cache';
  const PWA_STATE = {
    installing: false,
    requestId: null,
    total: 0,
  ...
  // Sync shim: ESM module editor/reading-mode.js calls this to keep the
  // legacy closure bool in sync when it owns the toggle logic.
  window.__setMainReadingMode = (v) => { isReadingMode = !!v; };
  ...
  // Shim so ESM settings/display.js can null this closure var when details hide.
  ...
  // 让 IIFE 内部仍以闭包名引用的调用点（如 applyI18n 与导入流程）继续工作。
  // 正式来源：window.FUDOKI_CONST + window.applyTheme / window.setThemePreference。
  const THEME = {
    PAPER: 'paper', SAKURA: 'sakura', STICKY: 'sticky',
    GREEN: 'green', BLUE: 'blue', DARK: 'dark', AUTO: 'auto'
  };
  const LIGHT_THEMES = [THEME.PAPER, THEME.SAKURA, THEME.STICKY, THEME.GREEN, THEME.BLUE];
  let savedThemePreference =
    (localStorage.getItem(LS.theme) || THEME.PAPER);
  const applyTheme = (pref) => {
    savedThemePreference = pref || savedThemePreference;
    return window.applyTheme && window.applyTheme(pref);
  };
  const setThemePreference = (pref) => {
    savedThemePreference = pref || savedThemePreference;
    return window.setThemePreference && window.setThemePreference(pref);
  };
  const syncThemeSelects = (pref) =>
    window.syncThemeSelects && window.syncThemeSelects(pref);
  ...
  const DEFAULT_CONTENT = `読み聞く庵 は、日本語テキストを簡単に分析できるWebアプリです。
...
Try YomiKiku-an and enjoy Japanese language analysis!`;
  ...
  // 词性解析函数
  // parsePartOfSpeech 移至 YomikikuanDict

  // 格式化详细信息
  // formatDetailInfo 移至 YomikikuanDict
  ...
  class DocumentManager {
    ...
    truncateTitle(title, maxLength = 60) {
      // 仅作安全上限；视觉截断交给 CSS（doc-item-title 的 -webkit-line-clamp）。
  ...
  // Delegates to modules/analyzer/local/text-preprocess.js. Inline fallback kept
  // for boot-race safety — analyzeText runs only after user input, but the dynamic
  // ESM import may still be unresolved on the very first interaction.
  function filterParentheses(text) {
    const mod = (typeof window !== 'undefined') ? window.YomikikuanTextPreprocess : null;
    if (mod && mod.filterParentheses) return mod.filterParentheses(text);
    if (text == null) return text;
    // 先处理全角括号
    const result = String(text).replace(/（([^）]+)）/g, (match, content) => {
      // 检查是否包含汉字
      const hasKanji = /[\u4E00-\u9FAF]/.test(content);
      // 检查是否包含英文字母
      const hasEnglish = /[a-zA-Z]/.test(content);
  ...
  window.playToken = function(text, event, tokenData) {
    ...
    if (resolvedToken && window.YomikikuanDict && window.YomikikuanDict.getTechOverride) {
      const techOverride = window.YomikikuanDict.getTechOverride(resolvedToken);
      ...
      const lineText = Array.from(tokens).map(token => {
        ...
        if (tokenDataAttr) {
          try {
            ...
            // 检查自定义词典，如果有自定义读音，优先使用
            if (window.YomikikuanDict && window.YomikikuanDict.getTechOverride) {
              const techOverride = window.YomikikuanDict.getTechOverride(tokenData);
              if (techOverride && techOverride.reading) {
                textToSpeak = techOverride.reading;
              }
            }
  ...
  // 文本框失焦且结构变化时自动解析
  // Delegates to modules/analyzer/local/text-preprocess.js. Inline fallback kept
  // for boot-race safety — first call is at boot (`let lastStructureSignature =
  // computeStructureSignature(...)`), which can race the dynamic ESM import.
  function computeStructureSignature(text) {
    const mod = (typeof window !== 'undefined') ? window.YomikikuanTextPreprocess : null;
    if (mod && mod.computeStructureSignature) return mod.computeStructureSignature(text);
    const s = (text || '').trim();
    if (!s) return '0|0';
    const lines = s.split(/\n+/).length;
  ...
  function initDisplayControls() {
    if (window.__ESM_DISPLAY_SETTINGS) return; // owned by static/js/modules/settings/display.js
    // 动态获取当前DOM中的控件引用
  ...
  // Analyzer text preprocessing helpers — registers window.YomikikuanTextPreprocess.
  import('/static/js/modules/analyzer/local/text-preprocess.js')
    .catch((err) => console.warn('[analyzer/local/text-preprocess] import failed', err));
  // Shared toolbar/settings HTML template — registers window.YomikikuanToolbarContent.
  import('/static/js/modules/ui/toolbar-content.js')
    .catch((err) => console.warn('[ui/toolbar-content] import failed', err));
  // Document search helpers — registers window.YomikikuanDocsSearch.
  import('/static/js/modules/docs/search.js')
    .catch((err) => console.warn('[docs/search] import failed', err));
  ...
  // Editor toolbar bootstrap is owned by modules/editor/editor-toolbar.js
  // (self-bootstraps on ESM import via app.js). The classic-script call here
  // was redundant and double-wired the click listener on editorStarToggle —
  // removed in favour of the ESM self-bootstrap.
...
// 高度调整功能
  ...
  function initReadingModeToggle() {
    if (window.__ESM_READING_MODE) return; // owned by static/js/modules/editor/reading-mode.js
  ...
  // 顶部编辑工具栏：implementation owned by modules/editor/editor-toolbar.js
  // (self-bootstraps via app.js → editor-toolbar.js, sets window.updateEditorToolbar).
  // This shim is hoisted so the four call sites in documentManager methods
  // (saveContent / copy-on-write / etc.) keep resolving their closure-bound
  // `updateEditorToolbar` reference. No-ops until the ESM module loads, but
  // those call sites are user-triggered (post-boot) so the ESM is always live.
  function updateEditorToolbar() {
    const w = (typeof window !== 'undefined') ? window : null;
    if (w && typeof w.updateEditorToolbar === 'function' && w.updateEditorToolbar !== updateEditorToolbar) {
      try { w.updateEditorToolbar(); } catch (_) {}
    }
  }
  ...
  // 创建共享工具栏内容HTML
  // Delegates to modules/ui/toolbar-content.js. Inline fallback kept for boot-race
  // safety — initSharedToolbarContent runs from initializeApp at boot, which can
  // race the dynamic ESM import on slow connections.
  function createToolbarContentHTML(context) {
    const mod = (typeof window !== 'undefined') ? window.YomikikuanToolbarContent : null;
    if (mod && mod.createToolbarContentHTML) return mod.createToolbarContentHTML(context, t);
  ...
  function initSearchModal() {
    ...
    // 渲染空状态
    // Empty-state markup delegates to modules/docs/search.js. Inline fallback
    // kept for boot-race safety (initSearchModal runs from initializeApp).
    function renderEmptyState() {
      if (!searchModalResults) return;
      const mod = (typeof window !== 'undefined') ? window.YomikikuanDocsSearch : null;
      if (mod && mod.buildEmptyStateMarkup) {
        searchModalResults.innerHTML = mod.buildEmptyStateMarkup('输入关键词开始搜索');
        return;
      }
    ...
    // 高亮关键词
    // Delegates to modules/docs/search.js; inline fallback kept for boot-race
    // safety (initSearchModal runs from initializeApp at boot, racing the
    // dynamic ESM import).
    function highlightText(text, query) {
      const mod = (typeof window !== 'undefined') ? window.YomikikuanDocsSearch : null;
      if (mod && mod.highlightText) return mod.highlightText(text, query);
    ...
    // 执行搜索
    // Filtering/snippet logic delegates to modules/docs/search.js.
    // Inline fallback preserved per the boot-race convention.
    function performSearch(query) {
      ...
      const mod = (typeof window !== 'undefined') ? window.YomikikuanDocsSearch : null;
      if (mod && mod.searchDocuments) {
        searchResults = mod.searchDocuments(
          docs,
          query,
          (content) => documentManager.getDocumentTitle(content)
        );
        renderResults(query);
        return;
      }

      // Inline fallback (boot-race only).
      ...
      searchResults = docs.filter(doc => {
        ...
        if (!snippet && text) snippet = text.substring(0, 120);
        return { id: doc.id, title: title || '无标题', snippet, createdAt: doc.createdAt };
      });
    ...
    function renderResults(query) {
      ...
      if (searchResults.length === 0) {
        if (mod && mod.buildEmptyStateMarkup) {
          searchModalResults.innerHTML = mod.buildEmptyStateMarkup('未找到匹配的文档');
        } else {
          // Inline fallback (boot-race only).
          searchModalResults.innerHTML = `
          <div class="search-empty-state">
      ...
      let html;
      if (mod && mod.buildResultsMarkup) {
        html = mod.buildResultsMarkup(searchResults, selectedIndex, query, highlightText);
      } else {
        // Inline fallback (boot-race only).
        html = searchResults.map((result, index) => {
  ...
  function initUserProfile() {
    ...
    // 检查用户登录状态
    const userDataStr = localStorage.getItem('yomikikuan_user');
    if (userDataStr) {
      try {
    ...
    // 切换账户功能（登录已移除：仅清理本地数据后刷新首页）
    switchAccountBtn.addEventListener('click', async () => {
      userProfileContainer.classList.remove('open');
      try {
        if (window.firebaseSignOut && typeof window.firebaseSignOut === 'function') {
          try { await window.firebaseSignOut(); } catch (_) {}
        }
        localStorage.removeItem('yomikikuan_user');
      } finally {
        window.location.href = 'index.html';
      }
    });

    // 登出功能（登录已移除：仅清理本地数据后刷新首页）
