 ...
 * 带 IndexedDB 持久化缓存（同一文本秒出）
 */

// ---- IndexedDB token cache (module-private) ----
const __SEG_IDB_NAME = 'yomikikuan-tokens';
...
function __segOpenIDB() {
  return new Promise((resolve, reject) => {
    try {
      const req = indexedDB.open(__SEG_IDB_NAME, __SEG_IDB_VER);
      req.onupgradeneeded = (e) => {
        const db = e.target.result;
        if (!db.objectStoreNames.contains(__SEG_IDB_STORE)) {
          const s = db.createObjectStore(__SEG_IDB_STORE, { keyPath: 'key' });
...
async function __segIdbGet(key) {
  try {
    const db = await __segOpenIDB();
    return await new Promise((resolve) => {
      const tx = db.transaction(__SEG_IDB_STORE, 'readonly');
      const req = tx.objectStore(__SEG_IDB_STORE).get(key);
      req.onsuccess = () => resolve(req.result || null);
      req.onerror = () => resolve(null);
    });
  } catch (_) { return null; }
}
async function __segIdbPut(key, lines) {
  try {
    const db = await __segOpenIDB();
    await new Promise((resolve) => {
      const tx = db.transaction(__SEG_IDB_STORE, 'readwrite');
      tx.objectStore(__SEG_IDB_STORE).put({ key, lines, ts: Date.now() });
      tx.oncomplete = resolve;
      tx.onerror = resolve;
    });
  } catch (_) {}
}
async function __segIdbPrune() {
  try {
    const db = await __segOpenIDB();
    const cutoff = Date.now() - __SEG_TTL_MS;
    const tx = db.transaction(__SEG_IDB_STORE, 'readwrite');
    const idx = tx.objectStore(__SEG_IDB_STORE).index('ts');
...
// cheap hash — cache key only, not security
function __segHash(str) {
