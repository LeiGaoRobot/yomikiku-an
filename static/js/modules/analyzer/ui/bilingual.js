...
import { callGemini as callGeminiShared, textBody, getApiKey } from '../../gemini/client.js';
...
async function callGeminiBatch(lines, signal) {
  const text = await callGeminiShared(
    textBody(buildPrompt(lines), { temperature: 0.2 }),
    { signal },
  );
  let arr;
  try {
