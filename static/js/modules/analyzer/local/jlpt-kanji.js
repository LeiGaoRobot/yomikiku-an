// JLPT kanji grade map. Source: davidluzgouveia/kanji-data (MIT), fetched 2026-04-21.
// Flattened from kanji.json using the `jlpt_new` field (N5 easiest .. N1 hardest).
// Only kanji where jlpt_new is non-null are included. Sorted by level (N5 first) then codepoint.

export const KANJI_GRADE = Object.freeze({
  // N5
  '一': 'N5',
  '七': 'N5',
  '万': 'N5',
  '三': 'N5',
  '上': 'N5',
  '下': 'N5',
  '中': 'N5',
  '九': 'N5',
  '二': 'N5',
  '五': 'N5',
  '人': 'N5',
  '今': 'N5',
  '休': 'N5',
  '何': 'N5',
  '先': 'N5',
  '入': 'N5',
  '八': 'N5',
  '六': 'N5',
  '円': 'N5',
  '出': 'N5',
  '前': 'N5',
  '北': 'N5',
  '十': 'N5',
  '千': 'N5',
  '午': 'N5',
  '半': 'N5',
  '南': 'N5',
  '友': 'N5',
  '右': 'N5',
  '名': 'N5',
  '四': 'N5',
  '国': 'N5',
  '土': 'N5',
  '外': 'N5',
  '大': 'N5',
  '天': 'N5',
  '女': 'N5',
  '子': 'N5',
  '学': 'N5',
  '小': 'N5',
  '山': 'N5',
  '川': 'N5',
  '左': 'N5',
  '年': 'N5',
  '後': 'N5',
  '日': 'N5',
  '時': 'N5',
  '書': 'N5',
  '月': 'N5',
  '木': 'N5',
  '本': 'N5',
  '来': 'N5',
  '東': 'N5',
  '校': 'N5',
  '母': 'N5',
  '毎': 'N5',
  '気': 'N5',
  '水': 'N5',
  '火': 'N5',
  '父': 'N5',
  '生': 'N5',
  '男': 'N5',
  '白': 'N5',
  '百': 'N5',
  '聞': 'N5',
  '行': 'N5',
  '西': 'N5',
  '見': 'N5',
  '話': 'N5',
  '語': 'N5',
  '読': 'N5',
  '車': 'N5',
  '金': 'N5',
  '長': 'N5',
  '間': 'N5',
  '雨': 'N5',
  '電': 'N5',
  '食': 'N5',
  '高': 'N5',

  // N4
  '不': 'N4',
  '世': 'N4',
  '主': 'N4',
  '事': 'N4',
  '京': 'N4',
  '仕': 'N4',
  '代': 'N4',
  '以': 'N4',
  '会': 'N4',
  '住': 'N4',
  '体': 'N4',
  '作': 'N4',
  '使': 'N4',
  '借': 'N4',
  '元': 'N4',
  '兄': 'N4',
  '公': 'N4',
  '写': 'N4',
  '冬': 'N4',
  '切': 'N4',
  '別': 'N4',
  '力': 'N4',
  '勉': 'N4',
  '動': 'N4',
  '医': 'N4',
  '去': 'N4',
  '口': 'N4',
  '古': 'N4',
  '台': 'N4',
  '同': 'N4',
  '味': 'N4',
  '品': 'N4',
  '員': 'N4',
  '問': 'N4',
  '図': 'N4',
  '地': 'N4',
  '堂': 'N4',
  '場': 'N4',
  '売': 'N4',
  '夏': 'N4',
  '夕': 'N4',
  '多': 'N4',
  '夜': 'N4',
  '妹': 'N4',
  '姉': 'N4',
  '始': 'N4',
  '字': 'N4',
  '安': 'N4',
  '室': 'N4',
  '家': 'N4',
  '少': 'N4',
  '屋': 'N4',
  '工': 'N4',
  '帰': 'N4',
  '広': 'N4',
  '店': 'N4',
  '度': 'N4',
  '建': 'N4',
  '弟': 'N4',
  '強': 'N4',
  '待': 'N4',
  '心': 'N4',
  '思': 'N4',
  '急': 'N4',
  '悪': 'N4',
  '意': 'N4',
  '手': 'N4',
  '持': 'N4',
  '教': 'N4',
  '文': 'N4',
  '料': 'N4',
  '新': 'N4',
  '方': 'N4',
  '旅': 'N4',
  '族': 'N4',
  '早': 'N4',
  '明': 'N4',
  '映': 'N4',
  '春': 'N4',
  '昼': 'N4',
  '曜': 'N4',
  '有': 'N4',
  '服': 'N4',
  '朝': 'N4',
  '業': 'N4',
  '楽': 'N4',
  '歌': 'N4',
  '止': 'N4',
  '正': 'N4',
  '歩': 'N4',
  '死': 'N4',
  '注': 'N4',
  '洋': 'N4',
  '海': 'N4',
  '漢': 'N4',
  '牛': 'N4',
  '物': 'N4',
  '特': 'N4',
  '犬': 'N4',
  '理': 'N4',
  '用': 'N4',
  '田': 'N4',
  '町': 'N4',
  '画': 'N4',
  '界': 'N4',
  '病': 'N4',
  '発': 'N4',
  '目': 'N4',
  '真': 'N4',
  '着': 'N4',
  '知': 'N4',
  '研': 'N4',
  '社': 'N4',
  '私': 'N4',
  '秋': 'N4',
  '究': 'N4',
  '空': 'N4',
  '立': 'N4',
  '答': 'N4',
  '紙': 'N4',
  '終': 'N4',
  '習': 'N4',
  '考': 'N4',
  '者': 'N4',
  '肉': 'N4',
  '自': 'N4',
  '色': 'N4',
  '花': 'N4',
  '英': 'N4',
  '茶': 'N4',
  '親': 'N4',
  '言': 'N4',
  '計': 'N4',
  '試': 'N4',
  '買': 'N4',
  '貸': 'N4',
  '質': 'N4',
  '赤': 'N4',
  '走': 'N4',
  '起': 'N4',
  '足': 'N4',
  '転': 'N4',
  '近': 'N4',
  '送': 'N4',
  '通': 'N4',
  '週': 'N4',
  '運': 'N4',
  '道': 'N4',
  '重': 'N4',
  '野': 'N4',
  '銀': 'N4',
  '開': 'N4',
  '院': 'N4',
  '集': 'N4',
  '青': 'N4',
  '音': 'N4',
  '題': 'N4',
  '風': 'N4',
  '飯': 'N4',
  '飲': 'N4',
  '館': 'N4',
  '駅': 'N4',
  '験': 'N4',
  '魚': 'N4',
  '鳥': 'N4',
  '黒': 'N4',

  // N3
  '与': 'N3',
  '両': 'N3',
  '乗': 'N3',
  '予': 'N3',
  '争': 'N3',
  '互': 'N3',
  '亡': 'N3',
  '交': 'N3',
  '他': 'N3',
  '付': 'N3',
  '件': 'N3',
  '任': 'N3',
  '伝': 'N3',
  '似': 'N3',
  '位': 'N3',
  '余': 'N3',
  '例': 'N3',
  '供': 'N3',
  '便': 'N3',
  '係': 'N3',
  '信': 'N3',
  '倒': 'N3',
  '候': 'N3',
  '値': 'N3',
  '偉': 'N3',
  '側': 'N3',
  '偶': 'N3',
  '備': 'N3',
  '働': 'N3',
  '優': 'N3',
  '光': 'N3',
  '全': 'N3',
  '共': 'N3',
  '具': 'N3',
  '内': 'N3',
  '冷': 'N3',
  '処': 'N3',
  '列': 'N3',
  '初': 'N3',
  '判': 'N3',
  '利': 'N3',
  '到': 'N3',
  '制': 'N3',
  '刻': 'N3',
  '割': 'N3',
  '加': 'N3',
  '助': 'N3',
  '努': 'N3',
  '労': 'N3',
  '務': 'N3',
  '勝': 'N3',
  '勤': 'N3',
  '化': 'N3',
  '単': 'N3',
  '危': 'N3',
  '原': 'N3',
  '参': 'N3',
  '反': 'N3',
  '収': 'N3',
  '取': 'N3',
  '受': 'N3',
  '号': 'N3',
  '合': 'N3',
  '向': 'N3',
  '君': 'N3',
  '否': 'N3',
  '吸': 'N3',
  '吹': 'N3',
  '告': 'N3',
  '呼': 'N3',
  '命': 'N3',
  '和': 'N3',
  '商': 'N3',
  '喜': 'N3',
  '回': 'N3',
  '因': 'N3',
  '困': 'N3',
  '園': 'N3',
  '在': 'N3',
  '報': 'N3',
  '増': 'N3',
  '声': 'N3',
  '変': 'N3',
  '夢': 'N3',
  '太': 'N3',
  '夫': 'N3',
  '失': 'N3',
  '好': 'N3',
  '妻': 'N3',
  '娘': 'N3',
  '婚': 'N3',
  '婦': 'N3',
  '存': 'N3',
  '宅': 'N3',
  '守': 'N3',
  '完': 'N3',
  '官': 'N3',
  '定': 'N3',
  '実': 'N3',
  '客': 'N3',
  '害': 'N3',
  '容': 'N3',
  '宿': 'N3',
  '寄': 'N3',
  '富': 'N3',
  '寒': 'N3',
  '寝': 'N3',
  '察': 'N3',
  '対': 'N3',
  '局': 'N3',
  '居': 'N3',
  '差': 'N3',
  '市': 'N3',
  '師': 'N3',
  '席': 'N3',
  '常': 'N3',
  '平': 'N3',
  '幸': 'N3',
  '幾': 'N3',
  '座': 'N3',
  '庭': 'N3',
  '式': 'N3',
  '引': 'N3',
  '当': 'N3',
  '形': 'N3',
  '役': 'N3',
  '彼': 'N3',
  '徒': 'N3',
  '得': 'N3',
  '御': 'N3',
  '必': 'N3',
  '忘': 'N3',
  '忙': 'N3',
  '念': 'N3',
  '怒': 'N3',
  '怖': 'N3',
  '性': 'N3',
  '恐': 'N3',
  '恥': 'N3',
  '息': 'N3',
  '悲': 'N3',
  '情': 'N3',
  '想': 'N3',
  '愛': 'N3',
  '感': 'N3',
  '慣': 'N3',
  '成': 'N3',
  '戦': 'N3',
  '戻': 'N3',
  '所': 'N3',
  '才': 'N3',
  '打': 'N3',
  '払': 'N3',
  '投': 'N3',
  '折': 'N3',
  '抜': 'N3',
  '抱': 'N3',
  '押': 'N3',
  '招': 'N3',
  '指': 'N3',
  '捕': 'N3',
  '掛': 'N3',
  '探': 'N3',
  '支': 'N3',
  '放': 'N3',
  '政': 'N3',
  '敗': 'N3',
  '散': 'N3',
  '数': 'N3',
  '断': 'N3',
  '易': 'N3',
  '昔': 'N3',
  '昨': 'N3',
  '晩': 'N3',
  '景': 'N3',
  '晴': 'N3',
  '暗': 'N3',
  '暮': 'N3',
  '曲': 'N3',
  '更': 'N3',
  '最': 'N3',
  '望': 'N3',
  '期': 'N3',
  '未': 'N3',
  '末': 'N3',
  '束': 'N3',
  '杯': 'N3',
  '果': 'N3',
  '格': 'N3',
  '構': 'N3',
  '様': 'N3',
  '権': 'N3',
  '横': 'N3',
  '機': 'N3',
  '欠': 'N3',
  '次': 'N3',
  '欲': 'N3',
  '歯': 'N3',
  '歳': 'N3',
  '残': 'N3',
  '段': 'N3',
  '殺': 'N3',
  '民': 'N3',
  '求': 'N3',
  '決': 'N3',
  '治': 'N3',
  '法': 'N3',
  '泳': 'N3',
  '洗': 'N3',
  '活': 'N3',
  '流': 'N3',
  '浮': 'N3',
  '消': 'N3',
  '深': 'N3',
  '済': 'N3',
  '渡': 'N3',
  '港': 'N3',
  '満': 'N3',
  '演': 'N3',
  '点': 'N3',
  '然': 'N3',
  '煙': 'N3',
  '熱': 'N3',
  '犯': 'N3',
  '状': 'N3',
  '猫': 'N3',
  '王': 'N3',
  '現': 'N3',
  '球': 'N3',
  '産': 'N3',
  '由': 'N3',
  '申': 'N3',
  '留': 'N3',
  '番': 'N3',
  '疑': 'N3',
  '疲': 'N3',
  '痛': 'N3',
  '登': 'N3',
  '皆': 'N3',
  '盗': 'N3',
  '直': 'N3',
  '相': 'N3',
  '眠': 'N3',
  '石': 'N3',
  '破': 'N3',
  '確': 'N3',
  '示': 'N3',
  '礼': 'N3',
  '祖': 'N3',
  '神': 'N3',
  '福': 'N3',
  '科': 'N3',
  '程': 'N3',
  '種': 'N3',
  '積': 'N3',
  '突': 'N3',
  '窓': 'N3',
  '笑': 'N3',
  '等': 'N3',
  '箱': 'N3',
  '米': 'N3',
  '精': 'N3',
  '約': 'N3',
  '組': 'N3',
  '経': 'N3',
  '給': 'N3',
  '絵': 'N3',
  '絶': 'N3',
  '続': 'N3',
  '緒': 'N3',
  '罪': 'N3',
  '置': 'N3',
  '美': 'N3',
  '老': 'N3',
  '耳': 'N3',
  '職': 'N3',
  '育': 'N3',
  '背': 'N3',
  '能': 'N3',
  '腹': 'N3',
  '舞': 'N3',
  '船': 'N3',
  '良': 'N3',
  '若': 'N3',
  '苦': 'N3',
  '草': 'N3',
  '落': 'N3',
  '葉': 'N3',
  '薬': 'N3',
  '術': 'N3',
  '表': 'N3',
  '要': 'N3',
  '規': 'N3',
  '覚': 'N3',
  '観': 'N3',
  '解': 'N3',
  '記': 'N3',
  '訪': 'N3',
  '許': 'N3',
  '認': 'N3',
  '誤': 'N3',
  '説': 'N3',
  '調': 'N3',
  '談': 'N3',
  '論': 'N3',
  '識': 'N3',
  '警': 'N3',
  '議': 'N3',
  '負': 'N3',
  '財': 'N3',
  '貧': 'N3',
  '責': 'N3',
  '費': 'N3',
  '資': 'N3',
  '賛': 'N3',
  '越': 'N3',
  '路': 'N3',
  '辞': 'N3',
  '込': 'N3',
  '迎': 'N3',
  '返': 'N3',
  '迷': 'N3',
  '追': 'N3',
  '退': 'N3',
  '逃': 'N3',
  '途': 'N3',
  '速': 'N3',
  '連': 'N3',
  '進': 'N3',
  '遅': 'N3',
  '遊': 'N3',
  '過': 'N3',
  '達': 'N3',
  '違': 'N3',
  '遠': 'N3',
  '適': 'N3',
  '選': 'N3',
  '部': 'N3',
  '都': 'N3',
  '配': 'N3',
  '酒': 'N3',
  '閉': 'N3',
  '関': 'N3',
  '降': 'N3',
  '限': 'N3',
  '除': 'N3',
  '険': 'N3',
  '陽': 'N3',
  '際': 'N3',
  '雑': 'N3',
  '難': 'N3',
  '雪': 'N3',
  '静': 'N3',
  '非': 'N3',
  '面': 'N3',
  '靴': 'N3',
  '頂': 'N3',
  '頭': 'N3',
  '頼': 'N3',
  '顔': 'N3',
  '願': 'N3',
  '類': 'N3',
  '飛': 'N3',
  '首': 'N3',
  '馬': 'N3',
  '髪': 'N3',
  '鳴': 'N3',

  // N2
  '並': 'N2',
  '丸': 'N2',
  '久': 'N2',
  '乱': 'N2',
  '乳': 'N2',
  '乾': 'N2',
  '了': 'N2',
  '介': 'N2',
  '仏': 'N2',
  '令': 'N2',
  '仲': 'N2',
  '伸': 'N2',
  '伺': 'N2',
  '低': 'N2',
  '依': 'N2',
  '個': 'N2',
  '倍': 'N2',
  '停': 'N2',
  '傾': 'N2',
  '像': 'N2',
  '億': 'N2',
  '兆': 'N2',
  '児': 'N2',
  '党': 'N2',
  '兵': 'N2',
  '冊': 'N2',
  '再': 'N2',
  '凍': 'N2',
  '刊': 'N2',
  '刷': 'N2',
  '券': 'N2',
  '刺': 'N2',
  '則': 'N2',
  '副': 'N2',
  '劇': 'N2',
  '効': 'N2',
  '勇': 'N2',
  '募': 'N2',
  '勢': 'N2',
  '包': 'N2',
  '匹': 'N2',
  '区': 'N2',
  '卒': 'N2',
  '協': 'N2',
  '占': 'N2',
  '印': 'N2',
  '卵': 'N2',
  '厚': 'N2',
  '双': 'N2',
  '叫': 'N2',
  '召': 'N2',
  '史': 'N2',
  '各': 'N2',
  '含': 'N2',
  '周': 'N2',
  '咲': 'N2',
  '喫': 'N2',
  '営': 'N2',
  '団': 'N2',
  '囲': 'N2',
  '固': 'N2',
  '圧': 'N2',
  '坂': 'N2',
  '均': 'N2',
  '型': 'N2',
  '埋': 'N2',
  '城': 'N2',
  '域': 'N2',
  '塔': 'N2',
  '塗': 'N2',
  '塩': 'N2',
  '境': 'N2',
  '央': 'N2',
  '奥': 'N2',
  '姓': 'N2',
  '委': 'N2',
  '季': 'N2',
  '孫': 'N2',
  '宇': 'N2',
  '宝': 'N2',
  '寺': 'N2',
  '封': 'N2',
  '専': 'N2',
  '将': 'N2',
  '尊': 'N2',
  '導': 'N2',
  '届': 'N2',
  '層': 'N2',
  '岩': 'N2',
  '岸': 'N2',
  '島': 'N2',
  '州': 'N2',
  '巨': 'N2',
  '巻': 'N2',
  '布': 'N2',
  '希': 'N2',
  '帯': 'N2',
  '帽': 'N2',
  '幅': 'N2',
  '干': 'N2',
  '幼': 'N2',
  '庁': 'N2',
  '床': 'N2',
  '底': 'N2',
  '府': 'N2',
  '庫': 'N2',
  '延': 'N2',
  '弱': 'N2',
  '律': 'N2',
  '復': 'N2',
  '快': 'N2',
  '恋': 'N2',
  '患': 'N2',
  '悩': 'N2',
  '憎': 'N2',
  '戸': 'N2',
  '承': 'N2',
  '技': 'N2',
  '担': 'N2',
  '拝': 'N2',
  '拾': 'N2',
  '挟': 'N2',
  '捜': 'N2',
  '捨': 'N2',
  '掃': 'N2',
  '掘': 'N2',
  '採': 'N2',
  '接': 'N2',
  '換': 'N2',
  '損': 'N2',
  '改': 'N2',
  '敬': 'N2',
  '旧': 'N2',
  '昇': 'N2',
  '星': 'N2',
  '普': 'N2',
  '暴': 'N2',
  '曇': 'N2',
  '替': 'N2',
  '札': 'N2',
  '机': 'N2',
  '材': 'N2',
  '村': 'N2',
  '板': 'N2',
  '林': 'N2',
  '枚': 'N2',
  '枝': 'N2',
  '枯': 'N2',
  '柔': 'N2',
  '柱': 'N2',
  '査': 'N2',
  '栄': 'N2',
  '根': 'N2',
  '械': 'N2',
  '棒': 'N2',
  '森': 'N2',
  '植': 'N2',
  '極': 'N2',
  '橋': 'N2',
  '欧': 'N2',
  '武': 'N2',
  '歴': 'N2',
  '殿': 'N2',
  '毒': 'N2',
  '比': 'N2',
  '毛': 'N2',
  '氷': 'N2',
  '永': 'N2',
  '汗': 'N2',
  '汚': 'N2',
  '池': 'N2',
  '沈': 'N2',
  '河': 'N2',
  '沸': 'N2',
  '油': 'N2',
  '況': 'N2',
  '泉': 'N2',
  '泊': 'N2',
  '波': 'N2',
  '泥': 'N2',
  '浅': 'N2',
  '浴': 'N2',
  '涙': 'N2',
  '液': 'N2',
  '涼': 'N2',
  '混': 'N2',
  '清': 'N2',
  '減': 'N2',
  '温': 'N2',
  '測': 'N2',
  '湖': 'N2',
  '湯': 'N2',
  '湾': 'N2',
  '湿': 'N2',
  '準': 'N2',
  '溶': 'N2',
  '滴': 'N2',
  '漁': 'N2',
  '濃': 'N2',
  '濯': 'N2',
  '灯': 'N2',
  '灰': 'N2',
  '炭': 'N2',
  '焼': 'N2',
  '照': 'N2',
  '燃': 'N2',
  '燥': 'N2',
  '爆': 'N2',
  '片': 'N2',
  '版': 'N2',
  '玉': 'N2',
  '珍': 'N2',
  '瓶': 'N2',
  '甘': 'N2',
  '畜': 'N2',
  '略': 'N2',
  '畳': 'N2',
  '療': 'N2',
  '皮': 'N2',
  '皿': 'N2',
  '省': 'N2',
  '県': 'N2',
  '短': 'N2',
  '砂': 'N2',
  '硬': 'N2',
  '磨': 'N2',
  '祈': 'N2',
  '祝': 'N2',
  '祭': 'N2',
  '禁': 'N2',
  '秒': 'N2',
  '移': 'N2',
  '税': 'N2',
  '章': 'N2',
  '童': 'N2',
  '競': 'N2',
  '竹': 'N2',
  '符': 'N2',
  '筆': 'N2',
  '筒': 'N2',
  '算': 'N2',
  '管': 'N2',
  '築': 'N2',
  '簡': 'N2',
  '籍': 'N2',
  '粉': 'N2',
  '粒': 'N2',
  '糸': 'N2',
  '紅': 'N2',
  '純': 'N2',
  '細': 'N2',
  '紹': 'N2',
  '絡': 'N2',
  '綿': 'N2',
  '総': 'N2',
  '緑': 'N2',
  '線': 'N2',
  '編': 'N2',
  '練': 'N2',
  '績': 'N2',
  '缶': 'N2',
  '署': 'N2',
  '群': 'N2',
  '羽': 'N2',
  '翌': 'N2',
  '耕': 'N2',
  '肌': 'N2',
  '肩': 'N2',
  '肯': 'N2',
  '胃': 'N2',
  '胸': 'N2',
  '脂': 'N2',
  '脳': 'N2',
  '腕': 'N2',
  '腰': 'N2',
  '膚': 'N2',
  '臓': 'N2',
  '臣': 'N2',
  '舟': 'N2',
  '航': 'N2',
  '般': 'N2',
  '芸': 'N2',
  '荒': 'N2',
  '荷': 'N2',
  '菓': 'N2',
  '菜': 'N2',
  '著': 'N2',
  '蒸': 'N2',
  '蔵': 'N2',
  '薄': 'N2',
  '虫': 'N2',
  '血': 'N2',
  '衣': 'N2',
  '袋': 'N2',
  '被': 'N2',
  '装': 'N2',
  '裏': 'N2',
  '補': 'N2',
  '複': 'N2',
  '角': 'N2',
  '触': 'N2',
  '訓': 'N2',
  '設': 'N2',
  '詞': 'N2',
  '詰': 'N2',
  '誌': 'N2',
  '課': 'N2',
  '諸': 'N2',
  '講': 'N2',
  '谷': 'N2',
  '豊': 'N2',
  '象': 'N2',
  '貝': 'N2',
  '貨': 'N2',
  '販': 'N2',
  '貯': 'N2',
  '貿': 'N2',
  '賞': 'N2',
  '賢': 'N2',
  '贈': 'N2',
  '超': 'N2',
  '跡': 'N2',
  '踊': 'N2',
  '軍': 'N2',
  '軒': 'N2',
  '軟': 'N2',
  '軽': 'N2',
  '輪': 'N2',
  '輸': 'N2',
  '辛': 'N2',
  '農': 'N2',
  '辺': 'N2',
  '述': 'N2',
  '逆': 'N2',
  '造': 'N2',
  '郊': 'N2',
  '郵': 'N2',
  '量': 'N2',
  '針': 'N2',
  '鈍': 'N2',
  '鉄': 'N2',
  '鉱': 'N2',
  '銅': 'N2',
  '鋭': 'N2',
  '録': 'N2',
  '門': 'N2',
  '防': 'N2',
  '陸': 'N2',
  '隅': 'N2',
  '階': 'N2',
  '隻': 'N2',
  '雇': 'N2',
  '雲': 'N2',
  '零': 'N2',
  '震': 'N2',
  '革': 'N2',
  '順': 'N2',
  '預': 'N2',
  '領': 'N2',
  '額': 'N2',
  '香': 'N2',
  '駐': 'N2',
  '骨': 'N2',
  '麦': 'N2',
  '黄': 'N2',
  '鼻': 'N2',
  '齢': 'N2',

  // N1
  '丁': 'N1',
  '丑': 'N1',
  '且': 'N1',
  '丘': 'N1',
  '丙': 'N1',
  '丞': 'N1',
  '丹': 'N1',
  '乃': 'N1',
  '之': 'N1',
  '乏': 'N1',
  '乙': 'N1',
  '也': 'N1',
  '亀': 'N1',
  '井': 'N1',
  '亘': 'N1',
  '亜': 'N1',
  '亥': 'N1',
  '亦': 'N1',
  '亨': 'N1',
  '享': 'N1',
  '亭': 'N1',
  '亮': 'N1',
  '仁': 'N1',
  '仙': 'N1',
  '仮': 'N1',
  '仰': 'N1',
  '企': 'N1',
  '伊': 'N1',
  '伍': 'N1',
  '伎': 'N1',
  '伏': 'N1',
  '伐': 'N1',
  '伯': 'N1',
  '伴': 'N1',
  '伶': 'N1',
  '伽': 'N1',
  '但': 'N1',
  '佐': 'N1',
  '佑': 'N1',
  '佳': 'N1',
  '併': 'N1',
  '侃': 'N1',
  '侍': 'N1',
  '侑': 'N1',
  '価': 'N1',
  '侮': 'N1',
  '侯': 'N1',
  '侵': 'N1',
  '促': 'N1',
  '俊': 'N1',
  '俗': 'N1',
  '保': 'N1',
  '修': 'N1',
  '俳': 'N1',
  '俵': 'N1',
  '俸': 'N1',
  '倉': 'N1',
  '倖': 'N1',
  '倣': 'N1',
  '倫': 'N1',
  '倭': 'N1',
  '倹': 'N1',
  '偏': 'N1',
  '健': 'N1',
  '偲': 'N1',
  '偵': 'N1',
  '偽': 'N1',
  '傍': 'N1',
  '傑': 'N1',
  '傘': 'N1',
  '催': 'N1',
  '債': 'N1',
  '傷': 'N1',
  '僕': 'N1',
  '僚': 'N1',
  '僧': 'N1',
  '儀': 'N1',
  '儒': 'N1',
  '償': 'N1',
  '允': 'N1',
  '充': 'N1',
  '克': 'N1',
  '免': 'N1',
  '典': 'N1',
  '兼': 'N1',
  '冒': 'N1',
  '冗': 'N1',
  '冠': 'N1',
  '冴': 'N1',
  '冶': 'N1',
  '准': 'N1',
  '凌': 'N1',
  '凜': 'N1',
  '凝': 'N1',
  '凡': 'N1',
  '凪': 'N1',
  '凱': 'N1',
  '凶': 'N1',
  '凸': 'N1',
  '凹': 'N1',
  '刀': 'N1',
  '刃': 'N1',
  '刈': 'N1',
  '刑': 'N1',
  '削': 'N1',
  '剖': 'N1',
  '剛': 'N1',
  '剣': 'N1',
  '剤': 'N1',
  '剰': 'N1',
  '創': 'N1',
  '功': 'N1',
  '劣': 'N1',
  '励': 'N1',
  '劾': 'N1',
  '勁': 'N1',
  '勅': 'N1',
  '勘': 'N1',
  '勧': 'N1',
  '勲': 'N1',
  '勺': 'N1',
  '匁': 'N1',
  '匠': 'N1',
  '匡': 'N1',
  '匿': 'N1',
  '升': 'N1',
  '卑': 'N1',
  '卓': 'N1',
  '博': 'N1',
  '卯': 'N1',
  '即': 'N1',
  '却': 'N1',
  '卸': 'N1',
  '厄': 'N1',
  '厘': 'N1',
  '厳': 'N1',
  '又': 'N1',
  '及': 'N1',
  '叔': 'N1',
  '叙': 'N1',
  '叡': 'N1',
  '句': 'N1',
  '只': 'N1',
  '叶': 'N1',
  '司': 'N1',
  '吉': 'N1',
  '后': 'N1',
  '吏': 'N1',
  '吐': 'N1',
  '吟': 'N1',
  '呂': 'N1',
  '呈': 'N1',
  '呉': 'N1',
  '哀': 'N1',
  '哉': 'N1',
  '哲': 'N1',
  '唄': 'N1',
  '唆': 'N1',
  '唇': 'N1',
  '唯': 'N1',
  '唱': 'N1',
  '啄': 'N1',
  '啓': 'N1',
  '善': 'N1',
  '喚': 'N1',
  '喝': 'N1',
  '喪': 'N1',
  '喬': 'N1',
  '嗣': 'N1',
  '嘆': 'N1',
  '嘉': 'N1',
  '嘱': 'N1',
  '器': 'N1',
  '噴': 'N1',
  '嚇': 'N1',
  '囚': 'N1',
  '圏': 'N1',
  '圭': 'N1',
  '坑': 'N1',
  '坪': 'N1',
  '垂': 'N1',
  '垣': 'N1',
  '執': 'N1',
  '培': 'N1',
  '基': 'N1',
  '堀': 'N1',
  '堅': 'N1',
  '堕': 'N1',
  '堤': 'N1',
  '堪': 'N1',
  '塀': 'N1',
  '塁': 'N1',
  '塊': 'N1',
  '塑': 'N1',
  '塚': 'N1',
  '塾': 'N1',
  '墓': 'N1',
  '墜': 'N1',
  '墨': 'N1',
  '墳': 'N1',
  '墾': 'N1',
  '壁': 'N1',
  '壇': 'N1',
  '壊': 'N1',
  '壌': 'N1',
  '士': 'N1',
  '壮': 'N1',
  '壱': 'N1',
  '奇': 'N1',
  '奈': 'N1',
  '奉': 'N1',
  '奎': 'N1',
  '奏': 'N1',
  '契': 'N1',
  '奔': 'N1',
  '奨': 'N1',
  '奪': 'N1',
  '奮': 'N1',
  '奴': 'N1',
  '如': 'N1',
  '妃': 'N1',
  '妄': 'N1',
  '妊': 'N1',
  '妙': 'N1',
  '妥': 'N1',
  '妨': 'N1',
  '姫': 'N1',
  '姻': 'N1',
  '姿': 'N1',
  '威': 'N1',
  '娠': 'N1',
  '娯': 'N1',
  '婆': 'N1',
  '婿': 'N1',
  '媒': 'N1',
  '媛': 'N1',
  '嫁': 'N1',
  '嫌': 'N1',
  '嫡': 'N1',
  '嬉': 'N1',
  '嬢': 'N1',
  '孔': 'N1',
  '孟': 'N1',
  '孤': 'N1',
  '宏': 'N1',
  '宗': 'N1',
  '宙': 'N1',
  '宜': 'N1',
  '宣': 'N1',
  '宥': 'N1',
  '宮': 'N1',
  '宰': 'N1',
  '宴': 'N1',
  '宵': 'N1',
  '寂': 'N1',
  '寅': 'N1',
  '密': 'N1',
  '寛': 'N1',
  '寡': 'N1',
  '寧': 'N1',
  '審': 'N1',
  '寮': 'N1',
  '寸': 'N1',
  '射': 'N1',
  '尉': 'N1',
  '尋': 'N1',
  '尚': 'N1',
  '尭': 'N1',
  '就': 'N1',
  '尺': 'N1',
  '尼': 'N1',
  '尽': 'N1',
  '尾': 'N1',
  '尿': 'N1',
  '屈': 'N1',
  '展': 'N1',
  '属': 'N1',
  '履': 'N1',
  '屯': 'N1',
  '岐': 'N1',
  '岬': 'N1',
  '岳': 'N1',
  '峠': 'N1',
  '峡': 'N1',
  '峰': 'N1',
  '峻': 'N1',
  '崇': 'N1',
  '崎': 'N1',
  '崚': 'N1',
  '崩': 'N1',
  '嵐': 'N1',
  '嵩': 'N1',
  '嵯': 'N1',
  '嶺': 'N1',
  '巌': 'N1',
  '巡': 'N1',
  '巣': 'N1',
  '巧': 'N1',
  '己': 'N1',
  '巳': 'N1',
  '巴': 'N1',
  '巽': 'N1',
  '帆': 'N1',
  '帝': 'N1',
  '帥': 'N1',
  '帳': 'N1',
  '幕': 'N1',
  '幣': 'N1',
  '幹': 'N1',
  '幻': 'N1',
  '幽': 'N1',
  '庄': 'N1',
  '序': 'N1',
  '庶': 'N1',
  '康': 'N1',
  '庸': 'N1',
  '廃': 'N1',
  '廉': 'N1',
  '廊': 'N1',
  '廷': 'N1',
  '弁': 'N1',
  '弊': 'N1',
  '弐': 'N1',
  '弓': 'N1',
  '弔': 'N1',
  '弘': 'N1',
  '弥': 'N1',
  '弦': 'N1',
  '弧': 'N1',
  '張': 'N1',
  '弾': 'N1',
  '彗': 'N1',
  '彦': 'N1',
  '彩': 'N1',
  '彪': 'N1',
  '彫': 'N1',
  '彬': 'N1',
  '彰': 'N1',
  '影': 'N1',
  '往': 'N1',
  '征': 'N1',
  '径': 'N1',
  '徐': 'N1',
  '従': 'N1',
  '循': 'N1',
  '微': 'N1',
  '徳': 'N1',
  '徴': 'N1',
  '徹': 'N1',
  '忌': 'N1',
  '忍': 'N1',
  '志': 'N1',
  '応': 'N1',
  '忠': 'N1',
  '怜': 'N1',
  '怠': 'N1',
  '怪': 'N1',
  '恒': 'N1',
  '恕': 'N1',
  '恨': 'N1',
  '恩': 'N1',
  '恭': 'N1',
  '恵': 'N1',
  '悌': 'N1',
  '悔': 'N1',
  '悟': 'N1',
  '悠': 'N1',
  '悦': 'N1',
  '悼': 'N1',
  '惇': 'N1',
  '惑': 'N1',
  '惜': 'N1',
  '惟': 'N1',
  '惣': 'N1',
  '惨': 'N1',
  '惰': 'N1',
  '愁': 'N1',
  '愉': 'N1',
  '愚': 'N1',
  '慈': 'N1',
  '態': 'N1',
  '慎': 'N1',
  '慕': 'N1',
  '慢': 'N1',
  '慧': 'N1',
  '慨': 'N1',
  '慮': 'N1',
  '慰': 'N1',
  '慶': 'N1',
  '憂': 'N1',
  '憤': 'N1',
  '憧': 'N1',
  '憩': 'N1',
  '憲': 'N1',
  '憶': 'N1',
  '憾': 'N1',
  '懇': 'N1',
  '懐': 'N1',
  '懲': 'N1',
  '懸': 'N1',
  '我': 'N1',
  '戒': 'N1',
  '戯': 'N1',
  '房': 'N1',
  '扇': 'N1',
  '扉': 'N1',
  '扱': 'N1',
  '扶': 'N1',
  '批': 'N1',
  '抄': 'N1',
  '把': 'N1',
  '抑': 'N1',
  '抗': 'N1',
  '択': 'N1',
  '披': 'N1',
  '抵': 'N1',
  '抹': 'N1',
  '抽': 'N1',
  '拍': 'N1',
  '拐': 'N1',
  '拒': 'N1',
  '拓': 'N1',
  '拘': 'N1',
  '拙': 'N1',
  '拠': 'N1',
  '拡': 'N1',
  '括': 'N1',
  '拳': 'N1',
  '拷': 'N1',
  '挑': 'N1',
  '挙': 'N1',
  '振': 'N1',
  '挿': 'N1',
  '据': 'N1',
  '捷': 'N1',
  '捺': 'N1',
  '授': 'N1',
  '掌': 'N1',
  '排': 'N1',
  '控': 'N1',
  '推': 'N1',
  '措': 'N1',
  '掲': 'N1',
  '描': 'N1',
  '提': 'N1',
  '揚': 'N1',
  '握': 'N1',
  '揮': 'N1',
  '援': 'N1',
  '揺': 'N1',
  '搬': 'N1',
  '搭': 'N1',
  '携': 'N1',
  '搾': 'N1',
  '摂': 'N1',
  '摘': 'N1',
  '摩': 'N1',
  '撃': 'N1',
  '撤': 'N1',
  '撮': 'N1',
  '撲': 'N1',
  '擁': 'N1',
  '操': 'N1',
  '擦': 'N1',
  '擬': 'N1',
  '攻': 'N1',
  '故': 'N1',
  '敏': 'N1',
  '救': 'N1',
  '敢': 'N1',
  '敦': 'N1',
  '整': 'N1',
  '敵': 'N1',
  '敷': 'N1',
  '斉': 'N1',
  '斎': 'N1',
  '斐': 'N1',
  '斗': 'N1',
  '斜': 'N1',
  '斤': 'N1',
  '斥': 'N1',
  '於': 'N1',
  '施': 'N1',
  '旋': 'N1',
  '旗': 'N1',
  '既': 'N1',
  '旦': 'N1',
  '旨': 'N1',
  '旬': 'N1',
  '旭': 'N1',
  '旺': 'N1',
  '昂': 'N1',
  '昆': 'N1',
  '昌': 'N1',
  '昭': 'N1',
  '是': 'N1',
  '昴': 'N1',
  '晃': 'N1',
  '晋': 'N1',
  '晏': 'N1',
  '晟': 'N1',
  '晨': 'N1',
  '晶': 'N1',
  '智': 'N1',
  '暁': 'N1',
  '暇': 'N1',
  '暉': 'N1',
  '暑': 'N1',
  '暖': 'N1',
  '暢': 'N1',
  '暦': 'N1',
  '暫': 'N1',
  '曙': 'N1',
  '曹': 'N1',
  '朋': 'N1',
  '朔': 'N1',
  '朕': 'N1',
  '朗': 'N1',
  '朱': 'N1',
  '朴': 'N1',
  '朽': 'N1',
  '杉': 'N1',
  '李': 'N1',
  '杏': 'N1',
  '杜': 'N1',
  '条': 'N1',
  '松': 'N1',
  '析': 'N1',
  '枠': 'N1',
  '枢': 'N1',
  '架': 'N1',
  '柄': 'N1',
  '柊': 'N1',
  '某': 'N1',
  '染': 'N1',
  '柚': 'N1',
  '柳': 'N1',
  '柾': 'N1',
  '栓': 'N1',
  '栗': 'N1',
  '栞': 'N1',
  '株': 'N1',
  '核': 'N1',
  '栽': 'N1',
  '桂': 'N1',
  '桃': 'N1',
  '案': 'N1',
  '桐': 'N1',
  '桑': 'N1',
  '桜': 'N1',
  '桟': 'N1',
  '梅': 'N1',
  '梓': 'N1',
  '梢': 'N1',
  '梧': 'N1',
  '梨': 'N1',
  '棄': 'N1',
  '棋': 'N1',
  '棚': 'N1',
  '棟': 'N1',
  '棺': 'N1',
  '椋': 'N1',
  '椎': 'N1',
  '検': 'N1',
  '椰': 'N1',
  '椿': 'N1',
  '楊': 'N1',
  '楓': 'N1',
  '楠': 'N1',
  '楼': 'N1',
  '概': 'N1',
  '榛': 'N1',
  '槙': 'N1',
  '槻': 'N1',
  '槽': 'N1',
  '標': 'N1',
  '模': 'N1',
  '樹': 'N1',
  '樺': 'N1',
  '橘': 'N1',
  '檀': 'N1',
  '欄': 'N1',
  '欣': 'N1',
  '欺': 'N1',
  '欽': 'N1',
  '款': 'N1',
  '歓': 'N1',
  '殉': 'N1',
  '殊': 'N1',
  '殖': 'N1',
  '殴': 'N1',
  '殻': 'N1',
  '毅': 'N1',
  '毬': 'N1',
  '氏': 'N1',
  '汁': 'N1',
  '汐': 'N1',
  '江': 'N1',
  '汰': 'N1',
  '汽': 'N1',
  '沖': 'N1',
  '沙': 'N1',
  '没': 'N1',
  '沢': 'N1',
  '沼': 'N1',
  '沿': 'N1',
  '泌': 'N1',
  '泡': 'N1',
  '泣': 'N1',
  '泰': 'N1',
  '洞': 'N1',
  '津': 'N1',
  '洪': 'N1',
  '洲': 'N1',
  '洵': 'N1',
  '洸': 'N1',
  '派': 'N1',
  '浄': 'N1',
  '浜': 'N1',
  '浦': 'N1',
  '浩': 'N1',
  '浪': 'N1',
  '浸': 'N1',
  '涯': 'N1',
  '淑': 'N1',
  '淡': 'N1',
  '淳': 'N1',
  '添': 'N1',
  '渇': 'N1',
  '渉': 'N1',
  '渋': 'N1',
  '渓': 'N1',
  '渚': 'N1',
  '渥': 'N1',
  '渦': 'N1',
  '湧': 'N1',
  '源': 'N1',
  '溝': 'N1',
  '滅': 'N1',
  '滉': 'N1',
  '滋': 'N1',
  '滑': 'N1',
  '滝': 'N1',
  '滞': 'N1',
  '漂': 'N1',
  '漆': 'N1',
  '漏': 'N1',
  '漠': 'N1',
  '漫': 'N1',
  '漬': 'N1',
  '漱': 'N1',
  '漸': 'N1',
  '潔': 'N1',
  '潜': 'N1',
  '潟': 'N1',
  '潤': 'N1',
  '潮': 'N1',
  '澄': 'N1',
  '澪': 'N1',
  '激': 'N1',
  '濁': 'N1',
  '濫': 'N1',
  '瀬': 'N1',
  '災': 'N1',
  '炉': 'N1',
  '炊': 'N1',
  '炎': 'N1',
  '為': 'N1',
  '烈': 'N1',
  '焦': 'N1',
  '煩': 'N1',
  '煮': 'N1',
  '熊': 'N1',
  '熙': 'N1',
  '熟': 'N1',
  '燎': 'N1',
  '燦': 'N1',
  '燿': 'N1',
  '爵': 'N1',
  '爽': 'N1',
  '爾': 'N1',
  '牧': 'N1',
  '牲': 'N1',
  '犠': 'N1',
  '狂': 'N1',
  '狩': 'N1',
  '独': 'N1',
  '狭': 'N1',
  '猛': 'N1',
  '猟': 'N1',
  '猪': 'N1',
  '献': 'N1',
  '猶': 'N1',
  '猿': 'N1',
  '獄': 'N1',
  '獣': 'N1',
  '獲': 'N1',
  '玄': 'N1',
  '率': 'N1',
  '玖': 'N1',
  '玲': 'N1',
  '珠': 'N1',
  '班': 'N1',
  '琉': 'N1',
  '琢': 'N1',
  '琳': 'N1',
  '琴': 'N1',
  '瑚': 'N1',
  '瑛': 'N1',
  '瑞': 'N1',
  '瑠': 'N1',
  '瑳': 'N1',
  '瑶': 'N1',
  '璃': 'N1',
  '環': 'N1',
  '甚': 'N1',
  '甫': 'N1',
  '甲': 'N1',
  '畔': 'N1',
  '畝': 'N1',
  '異': 'N1',
  '疎': 'N1',
  '疫': 'N1',
  '疾': 'N1',
  '症': 'N1',
  '痘': 'N1',
  '痢': 'N1',
  '痴': 'N1',
  '癒': 'N1',
  '癖': 'N1',
  '皇': 'N1',
  '皐': 'N1',
  '皓': 'N1',
  '盆': 'N1',
  '益': 'N1',
  '盛': 'N1',
  '盟': 'N1',
  '監': 'N1',
  '盤': 'N1',
  '盲': 'N1',
  '盾': 'N1',
  '眉': 'N1',
  '看': 'N1',
  '眸': 'N1',
  '眺': 'N1',
  '眼': 'N1',
  '睡': 'N1',
  '督': 'N1',
  '睦': 'N1',
  '瞬': 'N1',
  '瞭': 'N1',
  '瞳': 'N1',
  '矛': 'N1',
  '矢': 'N1',
  '矯': 'N1',
  '砕': 'N1',
  '砲': 'N1',
  '硝': 'N1',
  '硫': 'N1',
  '碁': 'N1',
  '碑': 'N1',
  '碧': 'N1',
  '碩': 'N1',
  '磁': 'N1',
  '磯': 'N1',
  '礁': 'N1',
  '礎': 'N1',
  '祉': 'N1',
  '祐': 'N1',
  '祥': 'N1',
  '票': 'N1',
  '禄': 'N1',
  '禅': 'N1',
  '禍': 'N1',
  '禎': 'N1',
  '秀': 'N1',
  '秘': 'N1',
  '租': 'N1',
  '秦': 'N1',
  '秩': 'N1',
  '称': 'N1',
  '稀': 'N1',
  '稔': 'N1',
  '稚': 'N1',
  '稜': 'N1',
  '稲': 'N1',
  '稼': 'N1',
  '稿': 'N1',
  '穀': 'N1',
  '穂': 'N1',
  '穏': 'N1',
  '穣': 'N1',
  '穫': 'N1',
  '穴': 'N1',
  '窃': 'N1',
  '窒': 'N1',
  '窮': 'N1',
  '窯': 'N1',
  '竜': 'N1',
  '竣': 'N1',
  '端': 'N1',
  '笙': 'N1',
  '笛': 'N1',
  '第': 'N1',
  '笹': 'N1',
  '筋': 'N1',
  '策': 'N1',
  '箇': 'N1',
  '節': 'N1',
  '範': 'N1',
  '篤': 'N1',
  '簿': 'N1',
  '粋': 'N1',
  '粗': 'N1',
  '粘': 'N1',
  '粛': 'N1',
  '糖': 'N1',
  '糧': 'N1',
  '系': 'N1',
  '糾': 'N1',
  '紀': 'N1',
  '紋': 'N1',
  '納': 'N1',
  '紗': 'N1',
  '紘': 'N1',
  '級': 'N1',
  '紛': 'N1',
  '素': 'N1',
  '紡': 'N1',
  '索': 'N1',
  '紫': 'N1',
  '紬': 'N1',
  '累': 'N1',
  '紳': 'N1',
  '紺': 'N1',
  '絃': 'N1',
  '結': 'N1',
  '絞': 'N1',
  '絢': 'N1',
  '統': 'N1',
  '絹': 'N1',
  '継': 'N1',
  '綜': 'N1',
  '維': 'N1',
  '綱': 'N1',
  '網': 'N1',
  '綸': 'N1',
  '綺': 'N1',
  '綾': 'N1',
  '緊': 'N1',
  '緋': 'N1',
  '締': 'N1',
  '緩': 'N1',
  '緯': 'N1',
  '縁': 'N1',
  '縄': 'N1',
  '縛': 'N1',
  '縦': 'N1',
  '縫': 'N1',
  '縮': 'N1',
  '繁': 'N1',
  '繊': 'N1',
  '織': 'N1',
  '繕': 'N1',
  '繭': 'N1',
  '繰': 'N1',
  '罰': 'N1',
  '罷': 'N1',
  '羅': 'N1',
  '羊': 'N1',
  '義': 'N1',
  '翁': 'N1',
  '翔': 'N1',
  '翠': 'N1',
  '翻': 'N1',
  '翼': 'N1',
  '耀': 'N1',
  '耐': 'N1',
  '耗': 'N1',
  '耶': 'N1',
  '聖': 'N1',
  '聡': 'N1',
  '聴': 'N1',
  '肇': 'N1',
  '肖': 'N1',
  '肝': 'N1',
  '肢': 'N1',
  '肥': 'N1',
  '肪': 'N1',
  '肺': 'N1',
  '胆': 'N1',
  '胎': 'N1',
  '胞': 'N1',
  '胡': 'N1',
  '胤': 'N1',
  '胴': 'N1',
  '脅': 'N1',
  '脈': 'N1',
  '脚': 'N1',
  '脩': 'N1',
  '脱': 'N1',
  '脹': 'N1',
  '腐': 'N1',
  '腸': 'N1',
  '膜': 'N1',
  '膨': 'N1',
  '臨': 'N1',
  '臭': 'N1',
  '至': 'N1',
  '致': 'N1',
  '興': 'N1',
  '舌': 'N1',
  '舎': 'N1',
  '舗': 'N1',
  '舜': 'N1',
  '舶': 'N1',
  '艇': 'N1',
  '艦': 'N1',
  '艶': 'N1',
  '芋': 'N1',
  '芙': 'N1',
  '芝': 'N1',
  '芳': 'N1',
  '芹': 'N1',
  '芽': 'N1',
  '苑': 'N1',
  '苗': 'N1',
  '茂': 'N1',
  '茄': 'N1',
  '茅': 'N1',
  '茉': 'N1',
  '茎': 'N1',
  '茜': 'N1',
  '荘': 'N1',
  '莉': 'N1',
  '莞': 'N1',
  '菊': 'N1',
  '菌': 'N1',
  '菖': 'N1',
  '菫': 'N1',
  '華': 'N1',
  '萌': 'N1',
  '萩': 'N1',
  '葬': 'N1',
  '葵': 'N1',
  '蒔': 'N1',
  '蒼': 'N1',
  '蓄': 'N1',
  '蓉': 'N1',
  '蓮': 'N1',
  '蔦': 'N1',
  '蕉': 'N1',
  '蕗': 'N1',
  '薦': 'N1',
  '薪': 'N1',
  '薫': 'N1',
  '藍': 'N1',
  '藤': 'N1',
  '藩': 'N1',
  '藻': 'N1',
  '蘭': 'N1',
  '虎': 'N1',
  '虐': 'N1',
  '虚': 'N1',
  '虜': 'N1',
  '虞': 'N1',
  '虹': 'N1',
  '蚊': 'N1',
  '蚕': 'N1',
  '蛇': 'N1',
  '蛍': 'N1',
  '蛮': 'N1',
  '蝶': 'N1',
  '融': 'N1',
  '衆': 'N1',
  '街': 'N1',
  '衛': 'N1',
  '衝': 'N1',
  '衡': 'N1',
  '衰': 'N1',
  '衷': 'N1',
  '衿': 'N1',
  '袈': 'N1',
  '裁': 'N1',
  '裂': 'N1',
  '裕': 'N1',
  '裟': 'N1',
  '裸': 'N1',
  '製': 'N1',
  '褐': 'N1',
  '褒': 'N1',
  '襟': 'N1',
  '襲': 'N1',
  '覆': 'N1',
  '覇': 'N1',
  '視': 'N1',
  '覧': 'N1',
  '訂': 'N1',
  '討': 'N1',
  '託': 'N1',
  '訟': 'N1',
  '訳': 'N1',
  '訴': 'N1',
  '診': 'N1',
  '証': 'N1',
  '詐': 'N1',
  '詔': 'N1',
  '評': 'N1',
  '詠': 'N1',
  '詢': 'N1',
  '詩': 'N1',
  '該': 'N1',
  '詳': 'N1',
  '誇': 'N1',
  '誉': 'N1',
  '誓': 'N1',
  '誕': 'N1',
  '誘': 'N1',
  '誠': 'N1',
  '誼': 'N1',
  '諄': 'N1',
  '請': 'N1',
  '諒': 'N1',
  '諭': 'N1',
  '諮': 'N1',
  '諾': 'N1',
  '謀': 'N1',
  '謁': 'N1',
  '謄': 'N1',
  '謙': 'N1',
  '謝': 'N1',
  '謡': 'N1',
  '謹': 'N1',
  '譜': 'N1',
  '譲': 'N1',
  '護': 'N1',
  '豆': 'N1',
  '豚': 'N1',
  '豪': 'N1',
  '貞': 'N1',
  '貢': 'N1',
  '貫': 'N1',
  '貴': 'N1',
  '賀': 'N1',
  '賃': 'N1',
  '賄': 'N1',
  '賊': 'N1',
  '賓': 'N1',
  '賜': 'N1',
  '賠': 'N1',
  '賦': 'N1',
  '購': 'N1',
  '赦': 'N1',
  '赳': 'N1',
  '赴': 'N1',
  '趣': 'N1',
  '距': 'N1',
  '跳': 'N1',
  '践': 'N1',
  '踏': 'N1',
  '躍': 'N1',
  '軌': 'N1',
  '軸': 'N1',
  '較': 'N1',
  '載': 'N1',
  '輔': 'N1',
  '輝': 'N1',
  '輩': 'N1',
  '轄': 'N1',
  '辰': 'N1',
  '辱': 'N1',
  '迅': 'N1',
  '迪': 'N1',
  '迫': 'N1',
  '迭': 'N1',
  '透': 'N1',
  '逐': 'N1',
  '逓': 'N1',
  '逝': 'N1',
  '逮': 'N1',
  '逸': 'N1',
  '遂': 'N1',
  '遇': 'N1',
  '遍': 'N1',
  '遣': 'N1',
  '遥': 'N1',
  '遭': 'N1',
  '遮': 'N1',
  '遵': 'N1',
  '遷': 'N1',
  '遺': 'N1',
  '遼': 'N1',
  '避': 'N1',
  '還': 'N1',
  '邑': 'N1',
  '那': 'N1',
  '邦': 'N1',
  '邪': 'N1',
  '邸': 'N1',
  '郁': 'N1',
  '郎': 'N1',
  '郡': 'N1',
  '郭': 'N1',
  '郷': 'N1',
  '酉': 'N1',
  '酌': 'N1',
  '酔': 'N1',
  '酢': 'N1',
  '酪': 'N1',
  '酬': 'N1',
  '酵': 'N1',
  '酷': 'N1',
  '酸': 'N1',
  '醜': 'N1',
  '醸': 'N1',
  '采': 'N1',
  '釈': 'N1',
  '釣': 'N1',
  '鈴': 'N1',
  '鉛': 'N1',
  '鉢': 'N1',
  '銃': 'N1',
  '銑': 'N1',
  '銘': 'N1',
  '銭': 'N1',
  '鋳': 'N1',
  '鋼': 'N1',
  '錘': 'N1',
  '錠': 'N1',
  '錦': 'N1',
  '錬': 'N1',
  '錯': 'N1',
  '鍛': 'N1',
  '鎌': 'N1',
  '鎖': 'N1',
  '鎮': 'N1',
  '鏡': 'N1',
  '鐘': 'N1',
  '鑑': 'N1',
  '閑': 'N1',
  '閣': 'N1',
  '閥': 'N1',
  '閲': 'N1',
  '闘': 'N1',
  '阻': 'N1',
  '阿': 'N1',
  '附': 'N1',
  '陛': 'N1',
  '陣': 'N1',
  '陥': 'N1',
  '陪': 'N1',
  '陰': 'N1',
  '陳': 'N1',
  '陵': 'N1',
  '陶': 'N1',
  '隆': 'N1',
  '隊': 'N1',
  '随': 'N1',
  '隔': 'N1',
  '障': 'N1',
  '隠': 'N1',
  '隣': 'N1',
  '隷': 'N1',
  '隼': 'N1',
  '雄': 'N1',
  '雅': 'N1',
  '雌': 'N1',
  '雛': 'N1',
  '離': 'N1',
  '雰': 'N1',
  '雷': 'N1',
  '需': 'N1',
  '霊': 'N1',
  '霜': 'N1',
  '霞': 'N1',
  '霧': 'N1',
  '露': 'N1',
  '靖': 'N1',
  '鞠': 'N1',
  '韻': 'N1',
  '響': 'N1',
  '項': 'N1',
  '須': 'N1',
  '頌': 'N1',
  '頑': 'N1',
  '頒': 'N1',
  '頻': 'N1',
  '顕': 'N1',
  '顧': 'N1',
  '颯': 'N1',
  '飢': 'N1',
  '飼': 'N1',
  '飽': 'N1',
  '飾': 'N1',
  '養': 'N1',
  '餓': 'N1',
  '馨': 'N1',
  '駄': 'N1',
  '駆': 'N1',
  '駒': 'N1',
  '駿': 'N1',
  '騎': 'N1',
  '騒': 'N1',
  '騰': 'N1',
  '驚': 'N1',
  '髄': 'N1',
  '鬼': 'N1',
  '魁': 'N1',
  '魂': 'N1',
  '魅': 'N1',
  '魔': 'N1',
  '鮎': 'N1',
  '鮮': 'N1',
  '鯉': 'N1',
  '鯛': 'N1',
  '鯨': 'N1',
  '鳩': 'N1',
  '鳳': 'N1',
  '鴻': 'N1',
  '鵬': 'N1',
  '鶏': 'N1',
  '鶴': 'N1',
  '鷹': 'N1',
  '鹿': 'N1',
  '麗': 'N1',
  '麟': 'N1',
  '麻': 'N1',
  '麿': 'N1',
  '黎': 'N1',
  '黙': 'N1',
  '黛': 'N1',
  '鼓': 'N1',
});

export function kanjiGrade(ch) {
  return KANJI_GRADE[ch] || null;
}
